...
| `fdtd-201.x` | Rectangular PEC cavity **TE₂₀₁** resonant frequency (higher-order mode) | Analytic Pozar §6.3 | **LIVE** — extracted f₂₀₁ offset **−0.117 %** on 24×4×16 grid (a=0.24 m, b=0.04 m, d=0.16 m) / 30 000 steps / 400-bin DFT scan in [1.40, 1.72] GHz; TE₂₀₁ is the **sole** mode in the scan band (TE₁₀₁ at 1.126 GHz is excluded below, TE₁₀₂ at 1.975 GHz above; n≥1 modes pushed >3.7 GHz by narrow b). Gate asserts **±2.5 %** (grid dispersion ~−0.9 % at ~19 cells/λ for f₂₀₁). Validates mode selectivity + higher-frequency dispersion. Strict ±0.5 % path: halve `dx` to 5 mm (~38 cells/λ). Q-factor extraction deferred. `#[ignore]`-gated (tests/cavity_higher_mode.rs). |
