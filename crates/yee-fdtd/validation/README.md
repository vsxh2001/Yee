...
| `fdtd-201` | Rectangular PEC cavity TE₁₀₁ resonant frequency | Analytic Pozar §6.3 | **LIVE** — achieved **±0.063 %** on 20×10×20 grid / 30 000 steps (E_y probe, 400-bin DFT scan). Gate asserts ±2.5 % (dispersion floor on 10 mm cells). Strict ±0.5 % refinement path: halve `dx` to 5 mm (~56 cells/λ) and double N to 60 000. Q-factor extraction (damped-mode fitting) deferred to fdtd-201.1. |
