...
- **Per-PR CI jobs must stay well under the GitHub 6-hour workflow hard limit — refinement-LOOP gates go to `heavy-weekly.yml`.** Run 369 (2026-07-14) was the first run in days to survive push-cancellation to completion, and the accreted FEM job (eig-003 sweep 2h22m hosted + two coupling gates + a BO loop) hit the 6 h axe — cancelling the whole run even though every other job was green. Hosted runners are ~3-4× slower than the dev box; budget per-PR jobs ≤ ~4 h measured-hosted, set `timeout-minutes` on multi-hour jobs (fail one job fast instead of poisoning the run), and move any gate that is a refinement loop over multi-minute solves (automesh convergence, coupling corrector, BO-over-FEM) to `heavy-weekly.yml`. Per-PR coverage of the same subsystem stays via its single-solve gates.
- **Convergence-criterion frequency bins must stop at ~0.96·f_max (ADR-0216).** `f_max` is the frequency the λ/20 mesh rule is *designed to* — the edge of mesh validity — and bins there are the least trustworthy by construction. The first `engine-automesh-002` run judged linear Δ|S| on the full band up to f_max and failed at Δ = 0.84 with a **healthy trajectory** (notch converging 1.0 %, depth stable): per-bin forensics localized ALL of it to 5.85–6.0 GHz — a spurious dip wandering between passes plus a non-physical **+1.05 dB** bin at exactly f_max. Both convergence loops' gates cap their criterion band below f_max; do the same in any new adaptive/refinement criterion. (The 5.9-GHz moving-dip root-cause is queued as FS.0b.2c, low priority — it sits outside any band a design flow should read.) Related: the full 3-pass graded convergence gate runs in **`heavy-weekly.yml`** (cron + workflow_dispatch), not per-PR CI — ~90 min release locally, multi-hour hosted.
- **Rastered geometry quantizes design knobs — gradient/Broyden loops need `GradedMeshOptions.snap_edges` (ADR-0218).** Point-sampled rasterization rounds every trace edge to the local cell, so a design-refinement fine model is a **staircase**: sm-em-001's first run measured three stub lengths spanning 34 µm reading the *identical* notch frequency, with Broyden oscillating inside one ~2.5 % step. `snap_edges` shifts the nearest node onto every trace-AABB edge (spread over 4 nodes/side; junction ratio ≤ 9/7; only the 8 affected cells rewritten — recomputing all widths from cumulative nodes perturbs coarse cells by an ULP and breaks the fixture's bit-equal-coarse probe placement). Snapping also halved the apparent coarse-model bias (2.22 % → 0.99 % at the same seed). Default `false` so the pinned graded-gate grids stay bit-identical; any optimizer-driven flow should turn it on.
- **`sparams::directional_transmission_db` (the S.12 single-ratio |S21|) assumes both runs launch the SAME incident wave — measured FALSE for strongly-reflecting DUTs (ADR-0204).** The automesh forensics caught the DUT's plane-A forward wave sitting +10…15 dB above the reference's (clean fits, residual ≤ 0.016, β on the HJ dispersion — NOT a fit artifact): a shunt-stub DUT reflects broadband (|Γ| ≈ 0.7 even at its passband shoulders) and the reflection re-pumps the imperfectly matched aperture source, so `fwd_B(dut)/fwd_B(ref)` read a non-physical +10.7 dB. For any DUT with substantial reflection, use the **launch-normalized double ratio** `|T_dut|/|T_ref|` with `T = fwd_B/fwd_A` per run (`sparams::forward_transfer`, R.2-validated) — each run normalizes by its own incident wave. `yee_engine::automesh::converge_two_port` does this; existing S.12-based gates pass because their asserts are notch-location/depth-shaped, but do not build new absolute-|S21| measurements on the single ratio. Related loop hygiene (same ADR): a convergence loop must hold everything the fixture sizes in cells constant in METRES across passes (margins, air height, `TwoPortBoardOptions::npml`, probe spacing), and its criterion must be LINEAR |ΔS| (dB deltas blow up at deep notches — 15.35 dB measured at a converged notch).

---

*Last updated: 2026-07-08, post FS.0a (ADR-0204: push-button meshing — `yee_engine::automesh` auto_dx rulebook + convergence loop, gate `engine-automesh-001`; see the new §10 launch-inequality gotcha above). — Prior note: 2026-06-02, post ci/heavy-tests-release-gate (+ the mom-001 release-gate before it): main CI had been RED/cancelled for 10+ commits because the no-default-features lint-test ran heavy solver tests in DEBUG. All are now `#[ignore]`'d and run in dedicated `--release` CI gate jobs — mom-001 dipole; fem-eig-001 (×4) + fem-eig-003 (×5) via a FEM-eigen gate (`--test-threads=1` MANDATORY: 5×~7.4 GB solves OOM a 16 GB runner otherwise); heterogeneous_substrate / ntff_dipole / subgrid via an FDTD-heavy gate. §8 + §10 updated. (Two pre-existing P2 doc nits left for a follow-up: `heterogeneous_substrate.rs` module docstring cites the old 80³ grid/coords vs actual 120³; `fem_eig_003_strict_absorption_floor_gate`'s assert message cites a stale `[-45,-35] dB` window.) — Prior note: post Filter Phase F1.1b.1 (ADR-0108): the first full-wave EM gate in the filter pipeline shipped (`fdtd-line-eeff-001` propagation ε_eff, `afd1eff`), after fixing a `voxelize_microstrip` ground/dielectric one-cell air-gap that had been dragging ε_eff ~20% low; plus a bounded Docker dev container (`scripts/yee-box.sh`, `6bcd026`) that makes heavy/FDTD work runnable locally without crashing the host. See the two new §10 gotchas above. — Prior note: post Phase 1.gui.6 (ADR-0081): VSWR circles on the Smith chart in both `yee-gui` (egui) and `yee-plotters`. `smith_vswr_circle_points` / `smith_vswr_circle_pts` compute constant-VSWR loci at ρ=(VSWR−1)/(VSWR+1); VSWR ∈ {1.5, 2, 3, 5, 10} drawn in muted light-blue-grey before data traces. Key implementation gotcha: the loop-closing element is stashed explicitly as `first=[ρ,0]` and pushed (rather than computing `cos(TAU)` which is not exactly `1.0` in IEEE 754 — this causes an exact-equality close test to fail). Review APPROVED after P1 fixes (bit-identical closure in yee-plotters + missing render smoke test). Update this file whenever a decision is made twice or a gotcha bites twice.*
