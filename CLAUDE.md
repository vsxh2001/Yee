...
*Last updated: 2026-05-27, post Phase 2.fdtd.py.4 (ADR-0077): `run_cpml_reflection()` / `CpmlReflectionResult`, `run_ntff_broadside()` / `NtffResult`, `run_dispersive_drude()` / `DispersiveDrudeResult` Python drivers shipped in yee-py — closes the gap between the ADR-0074 Rust gates and the Python API. `cpml001_run()` / `ntff001_run()` / `dispersive001_run()` promoted to `pub` in yee-validation (following `fdtd204_t_analytic` precedent). 9 pytest cases (type-identity + gate + repr smoke). Tutorial 14 added. Review P1 fixes: rustdoc prose cleanup (remove `/// pub for Python bindings` boilerplate), `isinstance(r, <Type>)` type-identity checks, `ratio_db > 0.0` positivity guard, ADR cross-references in tutorial See-also. Also this run: recovered 59 orphaned commits (Phase 2.fdtd.py.0–3, ADRs 0068–0076, tutorials 10–13, Smith-chart work, FEM wave-port chain) that previous sessions had committed to the local git proxy but not pushed to origin/main. Update this file whenever a decision is made twice or a gotcha bites twice.*
