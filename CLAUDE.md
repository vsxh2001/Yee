...
*Last updated: 2026-05-27, post Phase 1.validation.2 (ADR-0074): three long-standing `CaseStatus::Skipped` FDTD gates (cpml-001 CPML ≥30 dB reflection, ntff-001 NTFF broadside/endfire ≥20 dB, dispersive-001 Drude Fresnel ≤20% rel err) wired from hardcoded Skipped stubs to real physics in `yee-validation/src/lib.rs`, following the fdtd-202 inline-duplication pattern. Six `#[ignore]`-gated unit tests added (3–20 min release); two stale unit tests fixed. Stale `dt` capture in `dispersive001_run` corrected. ADR-0075 (Phase 2.fdtd.py.2 FDTD dipole-pattern Python driver + fdtd-203 gate) scoping docs pushed by concurrent agent — unimplemented, spec/plan/ADR-0075 in-tree. Session also saw detached-HEAD / stale-local-main recurrence (fixed via `git reset --hard`; gotcha already documented). Update this file whenever a decision is made twice or a gotcha bites twice.*
