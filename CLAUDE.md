...
- `.github/workflows/ci.yml` — Rust workspace lint + test on Linux + Rust 1.92. The no-default-features lint-test job runs `cargo check`, `cargo clippy -- -D warnings`, `cargo fmt --check`, `cargo test --workspace --no-default-features` (heavy solver gates — mom-001 dipole + the FDTD gates — are `#[ignore]`'d here and run as separate `--release` gate jobs; see §10), and `cargo doc --no-deps`. Includes a `python-bindings` job that runs `maturin develop` and `pytest` against `yee-py`. Installs `libfontconfig1-dev` and `pkg-config` so the plotters-backed crates build.
