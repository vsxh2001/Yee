...
*Last updated: 2026-05-28, post Phase 2.fdtd.9 (ADR-0078): `fdtd-205` Ohmic skin-depth penetration gate shipped in `crates/yee-fdtd/tests/ohmic_skin_depth.rs` and registered in `yee-validation`. 80×1×200 grid, E_y propagates in +x via H_z ↔ E_y leapfrog; conductor at x=50..79 (σ=2.533 S/m, δ=10 mm at 1 GHz). Gate A 5.31%<10%, Gate B 5.84%<15%; runs in ~2 s, NOT `#[ignore]`-gated. `SkinDepthResult` + `fdtd205_run()` registered in `run_all()` as `CaseStatus::Passed`. Key geometry lesson: the spec's original 5×5mm PEC box would place the TE₁₀ cutoff at 30 GHz >> 1 GHz (evanescent, kx=628/m >> 1/δ=100/m); the correct geometry propagates in x with NZ=200mm ensuring kz_min=15.7/m << 1/δ. Update this file whenever a decision is made twice or a gotcha bites twice.*
