...
- **`WavePort` (yee-mom): the cross-section eigenmode solver has SHIPPED.** Phase 1.3.1.1 (closed / slab-loaded guides — FR-4 §4 gate 1.39%) and Phase 1.3.1.2 (quasi-TEM microstrip — HJ 1.2%, ADR-0060) are both in. `ModalDistribution::Numerical2D` + `NumericalCrossSection::with_quasi_tem()` (ADR-0061) inject a numerical modal field, so a microstrip wave-port and a delta-gap excitation now produce **different** results. Remaining: the 2-D-cross-section→2.5-D-RWG **frame mapping** (the Phase-B residual — the diagnostic cross-section sits in the MoM `(x,y)` frame, not the physical transverse plane), and the fem-eig real-waveguide-port chain (deprioritized; modal-projection saturated — see the fem-eig-006 memory).
