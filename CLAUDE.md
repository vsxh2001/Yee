...
*Last updated: 2026-05-27, post Phase 2.fdtd.py.2 (ADR-0075): `run_dipole_pattern()` / `DipolePatternResult` Python driver shipped in yee-py (fdtd-203 short-dipole sin-θ NTFF radiation-pattern gate; Balanis §4.2); fdtd-203 registered in `Report::run_all()` as Skipped (wall-time ~30 s release); tutorial 12 added. P0 review finding addressed: `DipolePatternResult` and `run_dipole_pattern` were missing from `__init__.py` import/`__all__` — fixed before merge. Also landed in same session: Phase 1.validation.2 (ADR-0074) — cpml-001/ntff-001/dispersive-001 wired from Skipped stubs to real physics. Update this file whenever a decision is made twice or a gotcha bites twice.*
