...
*Last updated: 2026-05-28, post Phase 2.fdtd.9 (ADR-0078): `SkinDepthResult` + `fdtd205_run()` + `run_fdtd_205()` shipped in yee-validation — spatial complement to fdtd-202 (Q-factor temporal ring-down). Gate A (10 %): 1.05 %. Gate B (15 %): 2.22 %. Key finding: PMC y-faces (zero H_z at j=0/NY-1) required to suppress PEC-box evanescent TM₁₁ mode (cutoff ~42 GHz >> 1 GHz source); without PMC the evanescent decay (0.534/cell) dominates the Ohmic decay (0.905/cell) and the gate fails. Integration test `ohmic_skin_depth.rs` NOT `#[ignore]`-gated; runs in ~8 s debug. Python binding (Phase 2.fdtd.py.5) tracked as a follow-on. Review APPROVED (no P0/P1); P3 nits fixed pre-merge (σ sig-fig in description, import consistency, division guard uniformity, module-header precision). Update this file whenever a decision is made twice or a gotcha bites twice.*
