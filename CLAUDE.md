...
*Last updated: 2026-05-27, post Phase 2.fdtd.py.3 (ADR-0076): `run_fresnel_tfsf(nx,ny,nz,...)` / `FresnelTfsfResult` Python driver shipped in yee-py (fdtd-204 TF/SF Fresnel-transmission gate; Born-Wolf §1.6.2); first quantitative validation of `PlaneWaveSource` + `with_eps_r_cells` together; fdtd-204 registered in `Report::run_all()` as Skipped (wall-time ~5–15 min release); tutorial 13 added. `fdtd204_t_analytic()` is canonical analytic reference in yee-validation (pub); Python driver calls it directly. Review P1 findings fixed before merge: nx/ny/nz API (spec compliance), analytic-formula deduplication (no inline copy in yee-py), commit subject ≤72 chars. Update this file whenever a decision is made twice or a gotcha bites twice.*
