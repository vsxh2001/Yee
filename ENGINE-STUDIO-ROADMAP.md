...
converge, all over the job protocol; `engine-refine-001` joins CI. Next: multi-knob/BPF
refinement (yee-surrogate BO), directional S11, the antenna track. Before that,
S.11/F1.2.1.0 LOOP LANDED (ADR-0188): the closed
design loop (synthesize → engine-verify → fit → secant-correct → repeat) runs end-to-end;
the convergence gate is honestly deferred — the measured map oscillates on ~1 GHz-spaced
skirt bumps whose spacing matches the port-to-port round trip (the ADR-0129/0131
standing-wave artifact class). Named fix queued as S.12: 3-probe directional separation
(`fit_standing_wave`) on the S-parameter path. Before that, S.10 SHIPPED (ADR-0187): aperture ports on the engine.
