...
*Cross-section eigensolver milestone (Phase 1.3.1.1 steps 4–5.5, 2026-05-23):* the quasi-TEM cross-section (waveguide-port) eigensolver is now **published-benchmark-validated for inhomogeneous fills at FR-4** (§4 gate closed at ε_r=4.4, 1.39% vs the verified transcendental reference) on top of homogeneous (WR-90 0.055%) + uniform-fill (rel 1.5e-4) production accuracy, and now carries a **validated second-order (p=2) Nedelec/nodal element family** (J1 independent-quadrature pins + J3 homogeneous-TE10 anchor). Shipped this chain: block LOBPCG (step 4), complex LOBPCG (4.1), mixed (E_t,E_z) longitudinal block (5), verified slab-loaded reference (5.1), β-direct extraction fix (5.2), direct sparse shift-invert (5.3), interface-graded mesh + ε_r=10.2 h-plateau finding (5.4), p=2 elements + ε_r=10.2 mode-selection finding (5.5). The ε_r=10.2 high-contrast closure remains open — now narrowed past the element order to an out-of-lane mode-selection gap (step-5.6). Earlier stale "in-flight" labels resolved: mom-002 TEM-mode RHS port fix (Track WWWWWWW) shipped at merge `a08f0db` (mom-002 gate passes); step 5 shipped above.

*In-flight (this session):*
- Phase 1.3.1.1 step 5.6 (queued by step-5.5) — close the ε_r=10.2 high-contrast residual via a **p=2-robust mode selection** for `solve.rs::solve_dense_mixed`: the step-5.5 p=2 elements are validated (J1 independent-quadrature pins + J3 homogeneous-TE10 anchor) but the existing **dense cutoff-pencil** shift selection locks onto a non-dominant mode in the dense inhomogeneous p=2 spectrum (the curl-free gradient edge functions enlarge the near-null cluster), landing β≈438 (ε_eff≈4.8) instead of the reference 582.95 (ε_eff 8.17). Candidate fixes: seed the β-direct shift-invert from the physical β estimate / a TEM-like vector and screen by ε_eff rather than smallest cutoff k_c²; and/or a **sparse** cutoff selection so finer p=2 meshes (where higher-order convergence shows) become affordable (dense O(n³) selection caps p=2 at ≈6×6). Lane: `solve.rs` (the step-5.5 finding deliberately left this untouched — a solver change was out of the step-5.5 lane). Spec + plan + ADR not yet drafted.
