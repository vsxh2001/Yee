...
*Cross-section eigensolver milestone (Phase 1.3.1.1 steps 4–5.6, 2026-05-24):* the quasi-TEM cross-section (waveguide-port) eigensolver is now **published-benchmark-validated for inhomogeneous fills at FR-4** (§4 gate closed at ε_r=4.4, 1.39% vs the verified transcendental reference) on top of homogeneous (WR-90 0.055%) + uniform-fill (rel 1.5e-4, p=1 and p=2) production accuracy, and carries a **validated second-order (p=2) Nedelec/nodal element family** (J1 independent-quadrature pins + J3 homogeneous-TE10 anchor + the p=2 uniform-fill ε_r=2.55 anchor), reachable end-to-end via `NumericalCrossSection::with_element_order`. Shipped this chain: block LOBPCG (step 4), complex LOBPCG (4.1), mixed (E_t,E_z) longitudinal block (5), verified slab-loaded reference (5.1), β-direct extraction fix (5.2), direct sparse shift-invert (5.3), interface-graded mesh + ε_r=10.2 h-plateau finding (5.4), p=2 elements + ε_r=10.2 mode-selection finding (5.5), p=2-robust mode selection (5.6). The ε_r=10.2 high-contrast ≤5% closure remains open — now narrowed past *both* the element order *and* the mode selection to the first-order-class discretization rate at the high-contrast interface, requiring a finer p=2 `--release` mesh / sparse cutoff selection (step-5.7). Earlier stale "in-flight" labels resolved: mom-002 TEM-mode RHS port fix (Track WWWWWWW) shipped at merge `a08f0db` (mom-002 gate passes); step 5 shipped above.

*In-flight (this session):*
- Phase 1.3.1.1 step 5.7 (queued by step-5.6) — reach the ε_r=10.2 high-contrast ≤5% closure now that the selection is p=2-robust (step 5.6 recovered the physical dominant mode, ε_eff 4.77→5.807, but β plateaus at ≈486 / ε_eff ≈5.8 for both p=1 and p=2 because the dense O(n³) cutoff selection caps p=2 at ≈6×6). Candidate fixes: a **sparse** cutoff candidate gather (replace the dense `complex_eigenvalues()` in `solve.rs::cutoff_candidates` with a sparse shift-invert / LOBPCG sweep) so finer p=2 meshes — where the second-order convergence advantage would actually manifest at the high-contrast interface field peak — become affordable; plus a finer-`--release` p=2 example to demonstrate the convergence toward 582.95 (ε_eff 8.17). Lane: `solve.rs` (the cutoff-selection cost), `examples/`. Spec + plan + ADR not yet drafted.
