...
*Cross-section eigensolver milestone (Phase 1.3.1.1 steps 4–5.3, 2026-05-23):* the quasi-TEM cross-section (waveguide-port) eigensolver is now **published-benchmark-validated for inhomogeneous fills at FR-4** (§4 gate closed at ε_r=4.4, 1.39% vs the verified transcendental reference) on top of homogeneous (WR-90 0.055%) + uniform-fill (rel 1.5e-4) production accuracy. Shipped this chain: block LOBPCG (step 4), complex LOBPCG (4.1), mixed (E_t,E_z) longitudinal block (5), verified slab-loaded reference (5.1), β-direct extraction fix (5.2), direct sparse shift-invert (5.3). Earlier stale "in-flight" labels resolved: mom-002 TEM-mode RHS port fix (Track WWWWWWW) shipped at merge `a08f0db` (mom-002 gate passes); step 5 shipped above.

*In-flight (this session):*
- Phase 1.3.1.1 step 5.4 interface-graded mesh — close the ε_r=10.2 high-contrast residual (~16%, proven discretization-limited in 5.3) via interface-graded h-refinement (cheap-first; p-refinement is step-5.5 if h plateaus). Spec + plan + ADR-0055.
