...
- **Phase 1.3.1.1 step 5 mixed (E_t, E_z) longitudinal block for quasi-TEM wave-ports — SHIPPED 2026-05-23 (Track worktree-af50e6, pending merge `7272d0a`)**: `assemble_mixed` + `AssembledMixed` + `solve_dense_mixed` in `crates/yee-mom/src/eigensolver/` wire the staged Lee-Sun-Cendes longitudinal element matrices (`local_a_zz`/`local_b_zz`/`local_b_ze`) into the block pencil `A x = k_c² B x`, x=[E_t;E_z], consumed by `NumericalCrossSection::solve` (new `mode_profile_ez` field). The pencil is symmetric **indefinite** (B carries the edge-node coupling), so the solve uses the `B⁻¹A` non-symmetric path (nalgebra real-Schur, ~4 ms at n≈121) with SVD null-space eigenvectors + a transverse-energy spurious-mode filter — not Cholesky. Numerical `Z_w = (ωμ₀/β)·(∫|E_t|²/∫(1/μ_r)|E_t|²)` replaces the TE `η₀k₀/β` approximation, reducing to it on a homogeneous non-magnetic guide. DoD-V1 (homogeneous mixed β reproduces transverse β to rel err 4e-14 — the block-convention canary), DoD-V2′ (dielectric-slab-loaded β bracketed by the monotonic empty/full inequality + regression; transcendental ref deferred to step-5.1 per spec §7c escape hatch), DoD-V3 (Z_w reduces to TE form within 1%) green; `eigensolver_wr90` 2/2 + `wave_port_numerical_te10` unchanged. Zero new dependency. Spec + plan + ADR-0051.
